...
import com.erze.yqj.moudle.shortvideo.recordclip.RecordProgressController;
import com.erze.yqj.utils.FileUtils;
...
public class RecordActivity extends Activity implements
    ...
    public static final int MAX_DURATION = 1 * 30 * 1000;  //最长拍摄时长
    ...
    /*    if (bundle!=null){
            //初始化鉴权信息
            AuthInfoManager.getInstance().setAuthInfo(getApplicationContext(),bundle.getString(K1),bundle.getString(K2));
         *//*  //添加鉴权结果回调接口(不是必须)
...
*/
        ...
        mFlashView.getDrawable().setLevel(1);
...
//                    updateFaceunitParams();
    ...
    private void onFlashClick() {
        if ( mFlashView.getDrawable().getLevel()==2) {
           mKSYRecordKit.toggleTorch(false);
            mFlashView.getDrawable().setLevel(1);
        } else {
        mKSYRecordKit.toggleTorch(true);
            mFlashView.getDrawable().setLevel(2);
        }
    }
    ...
    private void initBeautyUI() {
        String[] items = new String[]{"禁用", "柔和", "美白", "魔幻",
                "磨皮", "温和", "自定义", "绿幕",
                    "复古","明亮", "胶片"};
